...
def _update_settings(source_folder, site_name):
    settings_path = source_folder + '/tourngen/settings.py'
    sed(settings_path, "DEBUG = True", "DEBUG = False")  #1
    sed(settings_path,
        'ALLOWED_HOSTS =.+$',
        'ALLOWED_HOSTS = ["%s"]' % (site_name,)  #2
    )
    secret_key_file = source_folder + '/tourngen/secret_key.py'
...
def _make_database_read_and_write(source_folder):
...
def deploy():
    ...
    _make_database_read_and_write(source_folder)